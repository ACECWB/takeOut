...
public class FrmBusCom extends JDialog implements ActionListener{
	...
	public void reloadTable(){
		try {
			...
			tblData =new Object[coms.size()][Commodity.BtableTitles.length];
			for(int i=0;i<coms.size();i++){
				for(int j=0;j<Commodity.BtableTitles.length;j++) {
					tblData[i][j] = coms.get(i).getBCell(j);
					
				}
			}
			tablmod.setDataVector(tblData,Commodity.BtableTitles);
	...
	public void reloadCommodityTable(){
		try {
			...
			tblData = new Object[commoditys.size()][Commodity.BtableTitles.length];
			for(int i=0;i<commoditys.size();i++){
				for(int j=0;j<Commodity.BtableTitles.length;j++) {
					tblData[i][j] = commoditys.get(i).getBCell(j); 
				}
			}
			tablmod.setDataVector(tblData, Commodity.BtableTitles);
			this.commodityTable.validate();
	...
	public FrmBusCom(Frame f, String s, boolean b) {
		super(f, s, b);
		...
		this.commodityTable.getModel().addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent e) {
				...
				if(c>=0 && r>=0) {
					...
					com.setBusinessId(Business.currentLoginBusiness.getBusinessId());
					com.setCounts(Integer.parseInt(tblData[i][4].toString()));
					com.setEachPrice(Float.parseFloat(tblData[i][5].toString()));
					com.setVipprice(Float.parseFloat(tblData[i][6].toString()));
					System.out.println(tblData[r][c].toString()+"  "+commodityTable.getValueAt(r, c));
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==this.btnAdd){
			...
			if(JOptionPane.showConfirmDialog(this,"确定删除该商品信息吗？","确认",JOptionPane.YES_NO_OPTION)==JOptionPane.YES_OPTION){
				...
				String businessid = Business.currentLoginBusiness.getBusinessId();
				try {
